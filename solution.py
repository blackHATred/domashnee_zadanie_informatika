def z6_22(number: int) -> None:
    ...
    print(f'б) Последняя цифра {str(number)[-1]} встречается {str(number).count(str(number)[-1])} раз(-а)')
    ...
    print(f'е) Цифры от 0 до 5 встречаются {len([i for i in str(number) if int(i) < 6])} раз(-а)')

def z6_23(number: int, a: int, x: int, y: int, z: int) -> None:
    print(f'а) Цифра {a} встречается {str(number).count(str(a))} раз(-а)')
...
def z6_25(numbers: tuple) -> None:
    print(f'Знак меняется {len([i for i in range(1, len(numbers)) if (numbers[i] == abs(numbers[i]) and numbers[i-1] != abs(numbers[i-1])) or (numbers[i] != abs(numbers[i]) and numbers[i-1] == abs(numbers[i-1]))])} раз(-а)')
